...
def fuzzy_team(name: str, known_teams: list) -> str:
...
for sport_key, data in cache.items():
    ...
    for fix in fixtures:
        ...
        for mkt_name, (true_prob, odds) in markets.items():
            ...
            if edge > 0.02 and true_prob > 0.60: # Edge > 2% AND Prob > 60%
                edges.append({
...
# Sort strictly by True Probability to find the "Safest" plays
edges.sort(key=lambda x: x["true_prob"], reverse=True)

print("--- SAFEST MATHEMATICAL BETS FOR TODAY ---")
for i, e in enumerate(edges[:10]):
    ...
    print(f"   True Prob: {e['true_prob']*100:.1f}% (Edge: +{e['edge']*100:.2f}%)")
    print()
